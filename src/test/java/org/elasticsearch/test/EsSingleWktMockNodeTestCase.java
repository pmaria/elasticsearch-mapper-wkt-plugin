...
import org.elasticsearch.plugin.mapper.geo.wkt.MapperWktPlugin;
...
/**
 * A copy of {@link org.elasticsearch.test.ESSingleNodeTestCase}.
 * A bit of a hack because {@link org.elasticsearch.test.ESSingleNodeTestCase}
 * is not extensible and I haven't yet found a way to inject the
 * {@link MapperWktPlugin} for use in tests.
 * 
 * Here we basically modify the newNode() method to return a
 * {@link MapperWktPlugin}-enabled {@link MockNode}
 *
 */
public class EsSingleWktMockNodeTestCase extends ESTestCase {
    ...
    private static void startNode() {
        assert NODE == null;
        NODE = newNode();
        // we must wait for the node to actually be up and running. otherwise
        // the node might have started, elected itself master but might not yet
        // have removed the
    ...
    /**
     * This method returns <code>true</code> if the node that is used in the
     * background should be reset after each test. This is useful if the test
     * changes the cluster state metadata etc. The default is <code>false</code>
     * .
     */
    ...
    /**
     * 
     * @return {@link MapperWktPlugin}-enabled {@link MockNode}
     * @author Pano Maria
     */
    private static Node newNode() {
        Settings nodeSettings = Settings.settingsBuilder()
                ...
                // This needs to tie into the ESIntegTestCase#indexSettings()
                // method
                .put("path.shared_data", createTempDir().getParent()).put("node.name", nodeName())
                .put(IndexMetaData.SETTING_NUMBER_OF_SHARDS, 1).put(IndexMetaData.SETTING_NUMBER_OF_REPLICAS, 0).put("script.inline", "on")
                .put("script.indexed", "on").put(EsExecutors.PROCESSORS, 1) // limit the number of threads created
                // make sure we get what we set :)
                .put("http.enabled", false).put(InternalSettingsPreparer.IGNORE_SYSTEM_PROPERTIES_SETTING, true)
                .build();

        // Pano: TODO:  Find a better way to inject specific plugins and stuff.
        Node build = new MockNode(nodeSettings, Version.V_2_3_2, list(MapperWktPlugin.class));
        build.start();
    ...
    /**
     * Helper method to populate classPathPlugin list.
     * 
     * @param classpathPlugin
     * @return
     * 
     * @author Pano Maria
     */
    ...
    /**
     * Get an instance for a particular class using the injector of the
     * singleton node.
     */
    ...
    /**
     * Create a new index on the singleton node with the provided index
     * settings.
     */
    ...
    protected static IndexService createIndex(String index, CreateIndexRequestBuilder createIndexRequestBuilder) {
        ...
        // Wait for the index to be allocated so that cluster state updates
        // don't override
        // changes that would have been done locally
        ClusterHealthResponse health = client().admin().cluster().health(
                Requests.clusterHealthRequest(index).waitForYellowStatus().waitForEvents(Priority.LANGUID).waitForRelocatingShards(0))
                .actionGet();
    ...
    /**
     * Ensures the cluster has a green state via the cluster health API. This
     * method will also wait for relocations. It is useful to ensure that all
     * action on the cluster have finished and all shards that were currently
     * relocating are now allocated and started.
     */
     ...
     *
     * @param timeout
     *            time out value to set on
     *            {@link org.elasticsearch.action.admin.cluster.health.ClusterHealthRequest}
     */
